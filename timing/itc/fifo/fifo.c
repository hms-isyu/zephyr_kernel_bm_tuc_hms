...
struct k_fifo test_fifo_self;
struct k_fifo test_fifo_high;
struct k_fifo test_fifo_low;
...
static void T_Self(void *p1, void *p2, void *p3)
{
  ...
  for (uint32_t step = 0U; step < MESSAGE_SIZE_STEPS; step++)
  {
    for (uint32_t i = 0U; i < MEASUREMENT_COUNT; i++)
    {
      ...
      k_fifo_put(&test_fifo_self, &test_item);
      rx_item = k_fifo_get(&test_fifo_self, K_FOREVER);
...
static void T_SendUp(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    BMTH_RESET_COUNTER();
    BMTH_GET_START_CNT(test_start_time);
    k_fifo_put(&test_fifo_high, &test_item);
  }
}
...
static void T_RecvUp(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    rx_item = k_fifo_get(&test_fifo_high, K_FOREVER);
    BMTH_GET_STOP_CNT(test_stop_time);
  }
}
...
static void T_SendDown(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    BMTH_RESET_COUNTER();
    BMTH_GET_START_CNT(test_start_time);
    k_fifo_put(&test_fifo_low, &test_item);
  }
}
...
static void T_RecvDown(void *p1, void *p2, void *p3)
{
  ...
  {
    k_sem_take(&test_recv_sem, K_FOREVER);
    rx_item = k_fifo_get(&test_fifo_low, K_FOREVER);
    BMTH_GET_STOP_CNT(test_stop_time);
  }
}
...
static void I_Task(void *p1, void *p2, void *p3)
{
  ...
  k_fifo_init(&test_fifo_self);
  k_fifo_init(&test_fifo_high);
  k_fifo_init(&test_fifo_low);
