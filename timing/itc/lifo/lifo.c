...
{
  void   *reserved;
  uint8_t payload[MESSAGE_SIZE_MAX];
} test_item_t;
...
static test_item_t test_item __aligned(4);
static test_item_t          *rx_item = NULL;

struct k_event test_event;
struct k_sem   test_arm_sem;
struct k_sem   test_recv_sem;

struct k_lifo test_lifo_self;
struct k_lifo test_lifo_high;
struct k_lifo test_lifo_low;
...
static void T_Self(void *p1, void *p2, void *p3)
{
  ...
  for (uint32_t step = 0U; step < MESSAGE_SIZE_STEPS; step++)
  {
    for (uint32_t i = 0U; i < MEASUREMENT_COUNT; i++)
    {
      ...
      k_lifo_put(&test_lifo_self, &test_item);
      rx_item = k_lifo_get(&test_lifo_self, K_FOREVER);
...
static void T_SendUp(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    BMTH_RESET_COUNTER();
    BMTH_GET_START_CNT(test_start_time);
    k_lifo_put(&test_lifo_high, &test_item);
  }
}
...
static void T_RecvUp(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    rx_item = k_lifo_get(&test_lifo_high, K_FOREVER);
    BMTH_GET_STOP_CNT(test_stop_time);
  }
}
...
static void T_SendDown(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    BMTH_RESET_COUNTER();
    BMTH_GET_START_CNT(test_start_time);
    k_lifo_put(&test_lifo_low, &test_item);
  }
}
...
static void T_RecvDown(void *p1, void *p2, void *p3)
{
  ...
  {
    k_sem_take(&test_recv_sem, K_FOREVER);
    rx_item = k_lifo_get(&test_lifo_low, K_FOREVER);
    BMTH_GET_STOP_CNT(test_stop_time);
  }
}
...
static void I_Task(void *p1, void *p2, void *p3)
{
  ...
  k_lifo_init(&test_lifo_self);
  k_lifo_init(&test_lifo_high);
  k_lifo_init(&test_lifo_low);
...
int main(void)
{
  ...
  {
    test_item.payload[i] = (uint8_t) (i + 1U);
  }
