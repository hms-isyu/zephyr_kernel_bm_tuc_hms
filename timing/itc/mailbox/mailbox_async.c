...
struct k_mbox test_mbox_self;
struct k_mbox test_mbox_high;
struct k_mbox test_mbox_low;

/* Written by the releaser only, where every measured thread is provably
 * blocked, and read by them from inside k_mbox_async_put() / k_mbox_get(). */
 ...
 * Arming the descriptors, opening and closing the windows, feeding the series
 * and stepping the sweep all belong to the releaser, which runs at priority 4 -
 * under every measured thread - and is therefore only ever scheduled once all
 * of them are blocked. None of that work can land inside a measurement window,
 * and the state it touches cannot be observed half-written.
 */

/*!
 * @brief Re-arm both descriptors for the next transaction.
 *
 * Called from the releaser only. The match overwrites rx_source_thread and info
 * on the receive descriptor, so they go back every time. tx_msg gets the same
 * treatment for consistency with the synchronous test, though
 * k_mbox_async_put() matches on a private copy and leaves this one untouched.
 *
...
static void T_Self(void *p1, void *p2, void *p3)
{
  ...
  for (uint32_t step = 0U; step < MESSAGE_SIZE_STEPS; step++)
  {
    ...
    for (uint32_t i = 0U; i < MEASUREMENT_COUNT; i++)
    {
      ...
      k_mbox_async_put(&test_mbox_self, &tx_msg, NULL);
      k_mbox_get(&test_mbox_self, &rx_msg, rx_buffer, K_FOREVER);
...
static void T_SendUp(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    BMTH_RESET_COUNTER();
    BMTH_GET_START_CNT(test_start_time);
    k_mbox_async_put(&test_mbox_high, &tx_msg, NULL);
  }
}
...
static void T_SendDown(void *p1, void *p2, void *p3)
{
  ...
  while (true)
  {
    ...
    BMTH_RESET_COUNTER();
    BMTH_GET_START_CNT(test_start_time);
    k_mbox_async_put(&test_mbox_low, &tx_msg, NULL);
  }
}
