...
/*!
 * \ingroup BMT_HMS
 * \brief Chain simulation for k_event_wait_safe() and z_swap.
 * @{
 * \file
 *
 * What is measured here are halves of the k_event_wait_safe() chain and of the
 * z_swap chain. Neither can be timed in the kernel itself: the window would
 * have to open inside the context switch. The simulation re-runs the same code
 * without a switch and times it.
 *
 * sim_z_swap_prologue mirrors arch_swap() as closely as the simulation allows
 * -- key stored, -EAGAIN stored, mask cleared, value reloaded -- and is
 * measured end to end by measure_event_z_swap_overhead(). It does not pend
 * PendSV -- the simulation must not actually switch -- so the SCB->ICSR write
 * of the real arch_swap() has no counterpart here.
 */
...
/* Desired mask for the wait chain: must NOT overlap sim_guard (the simulated
 * event->events), otherwise are_wait_conditions_met() succeeds and the pend /
 * swap path -- the chain under measurement -- is never entered. */
