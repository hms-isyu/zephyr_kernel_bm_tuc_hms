...
/*! S_B: tail of k_event_post() for a post with no matching waiter, which
 *  therefore never reaches the scheduler. */
void measure_event_tail_overhead(BMTH_measurement_series_t *mseries,
                                 uint32_t loop_count);

/*! S_A: one half of the z_swap chain on its own, selected by \p part. */
void measure_event_z_swap_overhead(BMTH_measurement_series_t *mseries,
                                   uint32_t          loop_count,
                                   sim_z_swap_part_t part);

/*! S_A: one half of the k_event_wait_safe() chain, selected by \p part. */
