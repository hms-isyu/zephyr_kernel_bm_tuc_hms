...
static void H_Task_SB(void *p1, void *p2, void *p3)
{
  ...
  while (1)
  {
    ...
    k_event_post(&test_event, SCENARIO_B_EVENT_MASK);
    BMTH_GET_STOP_CNT(test_stop_time);
...
static void I_Task(void *p1, void *p2, void *p3)
{
  ...
  dummy_task_event_mask = SCENARIO_B_EVENT_MASK;
...
int main(void)
{
  ...
  /* S_B: post with no matching waiter -> never reaches the scheduler. */
  measure_event_tail_overhead(&event_tail, MEASUREMENT_COUNT);

  /* S_A: the wait chain after the IRQ, where the thread gets the CPU back. */
  measure_wait_overhead(&wait_tail, MEASUREMENT_COUNT, SIM_WAIT_TAIL);

  /* S_A: the swap chain up to the irq_unlock() where PendSV would fire. */
